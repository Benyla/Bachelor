...
def parse_model_name(model_str): # returns use_adv flag and latent dimension
...
def main():
    ...
    for idx in args.indices:
        if idx < 0 or idx >= len(test_files):
            raise IndexError(f"Index {idx} is out of bounds (0–{len(test_files)-1})")
    ...
    # Load images & compute reconstructions
    originals, recons = [], []
    for idx in args.indices:
        path = test_files[idx]
